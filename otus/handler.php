<?php
file_put_contents($_SERVER['DOCUMENT_ROOT'].'/otus/rest_log.txt', var_export($_REQUEST, true), FILE_APPEND);
...
class DealImporter
{
    ...
    private static function sendRequest(string $endpoint, array $data): array
    {
        ...
        if ($response === false) {
            $error = curl_error($ch);
            file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/otus/rest_log_new.log', var_export('ОШИБКА: ' . $error, true), FILE_APPEND);
            curl_close($ch);
        }
